...
| 组件范围 | 尽量全量（去掉原项目业务定制组件与移动端，约 45 个） |
| 双端 | 只做 PC 端，不做移动端 |
...
| 高级 (6) | 上传、富文本（wangEditor React 版）、签名板、图标选择器（复用 ZaIcon 体系）、JSON 编辑、金额输入 |
| 布局 (8) | Row/Col、卡片、分割线、折叠面板、标签页、Space、描述列表、Flex |
| 子表单 (3) | SubForm（嵌套对象）、TableForm（antd Table 行内编辑数组）、StepForm |
| 图表 (3) | 柱状 / 折线 / 饼图（echarts 按需引入，走 dataviz 配色规范） |
| 辅助 (8) | 文本、段落、Alert 提示块、按钮、链接、图片、二维码、HTML |
...
| P3 布局+嵌套 | 8 个布局组件、容器嵌套拖拽 | 卡片套 Row 套 Col 套输入框正常工作 |
| P4 后端闭环 | form 表 + CRUD + 三个演示页 | 列表→设计→保存→渲染测试全链路通 |
| P5 高级+子表单 | 上传/富文本/签名/图表/SubForm/TableForm/StepForm | 子表单提交嵌套数据结构正确 |
...
- 前端：`@wangeditor/editor` + `@wangeditor/editor-for-react`（P5）、`echarts`（P5）、`qrcode`（P5）、`signature_pad`（P5）。拖拽复用已有 `@dnd-kit/react`，状态复用 zustand，不引入表单引擎/低代码框架。
